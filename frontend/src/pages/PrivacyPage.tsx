  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-[#f5f5f7]">
      ...
      <main className="mx-auto max-w-3xl px-6 pb-24 pt-32">
        ...
        <p className="mt-2 text-sm text-white/30">{t("privacy.updated", { date: new Date().toLocaleDateString(i18n.language) })}</p>
        <p className="mt-6 text-sm leading-relaxed text-white/50">{t("privacy.intro")}</p>

        <div className="mt-10 space-y-8">
          {(["one", "two", "three", "four", "five", "six"] as const).map((k) => (
            <section key={k}>
              <h2 className="text-lg font-medium text-white/85">{t(`privacy.sections.${k}.title`)}</h2>
              <p className="mt-2 text-sm leading-relaxed text-white/45">{t(`privacy.sections.${k}.body`)}</p>
            </section>
          ))}
        </div>
      </main>
